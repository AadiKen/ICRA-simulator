...
from train_portable_ppo import EPISODE_COLUMNS,episode_metric_row
from sb3_contrib import RecurrentPPO
from stable_baselines3.common.callbacks import BaseCallback
from stable_baselines3.common.vec_env import SubprocVecEnv
from stable_baselines3.common.vec_env.subproc_vec_env import _stack_obs

OUT=ROOT/"artifacts/rl-campaign/gazebo-correctness-dry-run"
...
def main(argv=None):
 ...
 model=RecurrentPPO("MlpLstmPolicy",env,seed=7319,n_steps=32,batch_size=64,n_epochs=1,learning_rate=3e-4,gamma=.99,gae_lambda=.95,clip_range=.2,ent_coef=.005,policy_kwargs={"net_arch":[64,64],"lstm_hidden_size":64,"enable_critic_lstm":True},verbose=0,device="cpu")
 ...
 checkpoint=OUT/"recurrent-ppo-correctness-only";model.save(checkpoint);env.close();checkpoint=checkpoint.with_suffix(".zip");announce(f"checkpoint saved: {checkpoint.relative_to(ROOT)}")

 announce("starting recurrent evaluation with LSTM state and episode_start threading")
 loaded=RecurrentPPO.load(checkpoint,device="cpu");evaluation_env=make(50000);eval_started=time.perf_counter();total=0.;state=None;episode_start=np.ones((1,),bool);threading=[]
 try:
  ...
  for controls in range(1,30):
   threading.append({"control":controls,"episode_start":bool(episode_start[0]),"state_was_none":state is None})
   action,state=loaded.predict(obs,state=state,episode_start=episode_start,deterministic=True);obs,reward,terminated,truncated,info=evaluation_env.step(action);total+=float(reward);episode_start=np.asarray([terminated or truncated],bool)
   if terminated or truncated:break
  row=episode_metric_row(seed=50000,env=evaluation_env,info=info,total_return=total,wall_clock_s=time.perf_counter()-eval_started,policy_id="RecurrentPPO-correctness-only",algorithm="RecurrentPPO")
