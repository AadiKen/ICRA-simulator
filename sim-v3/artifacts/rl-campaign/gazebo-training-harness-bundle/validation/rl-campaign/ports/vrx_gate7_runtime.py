...
class Runtime(Node):
    def __init__(self,schedule,output):
        ...
        self.i=0; self.imu=None; self.gps=None; self.port_angvel=None; self.starboard_angvel=None
        ...
        self.create_subscription(Float64,'/surveyor/thrusters/port/thrust/ang_vel',self.on_port_angvel,20)
        self.create_subscription(Float64,'/surveyor/thrusters/starboard/thrust/ang_vel',self.on_starboard_angvel,20)
    ...
    def on_port_angvel(self,msg): self.port_angvel=msg.data
    def on_starboard_angvel(self,msg): self.starboard_angvel=msg.data
    def on_gps(self,msg):
    ...
    def on_odom(self,msg):
        ...
        pitch_term=2*(q.w*q.y-q.z*q.x)
        pitch=math.copysign(math.pi/2,pitch_term) if abs(pitch_term)>=1 else math.asin(pitch_term)
        yaw_enu=math.atan2(2*(q.w*q.z+q.x*q.y),1-2*(q.y*q.y+q.z*q.z)); yaw=math.pi/2-yaw_enu
        v=msg.twist.twist.linear; w=msg.twist.twist.angular; imu=self.imu; now=self.i*self.schedule['physics_dt_s']; imu_sample=None
        if imu is not None:
        ...
        diagnostic_6dof={'position_enu_m':[msg.pose.pose.position.x,msg.pose.pose.position.y,msg.pose.pose.position.z],'quaternion_xyzw':[q.x,q.y,q.z,q.w],'rpy_enu_rad':[roll,pitch,yaw_enu],'linear_velocity_body_flu_mps':[v.x,v.y,v.z],'angular_velocity_body_flu_rad_s':[w.x,w.y,w.z]}
        diagnostic_6dof['finite']=all(math.isfinite(value) for values in diagnostic_6dof.values() if isinstance(values,list) for value in values)
        row={'time_s':now,'state':[msg.pose.pose.position.y,msg.pose.pose.position.x,yaw,math.cos(yaw)*v.y+math.sin(yaw)*v.x,-math.sin(yaw)*v.y+math.cos(yaw)*v.x,-w.z],'diagnostic_6dof':diagnostic_6dof,'imu':imu_sample,'gps':self.gps_model.sample(now),'thruster_newtons':[left,right],'thruster_feedback_angvel_rad_s':[self.port_angvel,self.starboard_angvel]}
