...
from .common_task import CompletionTracker, classify_termination, compute_reward, cross_track_distance, passed_waypoint_plane
...
class CommonWaypointEnv(gym.Env):
 ...
 def __init__(self,repository:str|Path,base_seed:int=0,fixed_reset_seed:int|None=None,final_leg_curriculum:bool=False,shaping_enabled:bool=True,bridge=None,backend_type:str="node",disturbance_mode:str="seeded",condition_contract_path:str|Path|None=None):
  self.root=Path(repository);common,self.contract_binding=load_native_task_contract(self.root,condition_contract_path);self.contract=common;self.final_radius_m=float(common["learnability"]["absolute_success_rate_threshold"]["terminal_definition"]["radius_m"]);self.terminal_variant=common["learnability"]["absolute_success_rate_threshold"]["terminal_definition"]["variant"];timing=common["timing"];self.timeout_steps=int(timing["episode_length_steps"]);self.physics_timestep_s=float(timing["physics_timestep_s"]);self.control_interval_s=float(timing["control_interval_s"]);ratio=self.control_interval_s/self.physics_timestep_s;self.physics_steps_per_action=round(ratio)
 ...
 def _randomization(self,seed):
  ...
  current=[speed*math.cos(direction),speed*math.sin(direction),0];wind_vector=[wind*math.cos(wd),wind*math.sin(wd),0]
  if self.disturbance_mode=="zero":current=[0.,0.,0.];wind_vector=[0.,0.,0.]
  return start,heading,route,current,wind_vector
 def _config(self,seed):
  start,heading,self.route,current,wind=self._randomization(seed);self.start=start
  return{"schema_version":1,"experiment":{"name":f"p3-v3-surveyor-{seed}","seed":seed,"timestep_s":self.physics_timestep_s,"duration_s":self.timeout_steps*self.physics_timestep_s},"backend":{"type":self.backend_type},"vehicle":{"preset":self.vehicle_preset,"plant":"planar3"},"environment":{"current_mps":current,"wind_mps":wind},"initial_state":{"position_ned_m":[*start,0],"attitude_rad":[0,0,heading]},"mission":{"type":"rl-common-waypoint-v1","waypoints":[{"north_m":19000,"east_m":19000}]},"sensors":[{"plugin":"imu","enabled":True},{"plugin":"gps","enabled":True}]}
 def reset(self,*,seed=None,options=None):
  super().reset(seed=seed);actual=self.fixed_reset_seed if self.fixed_reset_seed is not None else self.base_seed+self.episode;self.episode+=1;config=self._config(actual);reset_result=self.bridge.reset([config]);self.last_observation=reset_result["observations"][0];self.waypoint=2 if self.final_leg_curriculum else 0;self.prev_action[:]=0;self.steps=0;self.control_steps=0;self.hold=0;self.cross_track_sum=0;self.last_truth=self.bridge.ground_truth();self.prev_distance=self._distance();metric_route=self.route[self.waypoint:] if self.final_leg_curriculum else self.route;metric_start=self.route[self.waypoint-1] if self.final_leg_curriculum else self.start;self.completion_tracker=CompletionTracker.for_route(metric_start,metric_route);return self._obs(),{"seed":actual,"curriculum_phase":"final-leg-isolation" if self.final_leg_curriculum else "full-episode","control_interval_s":self.control_interval_s,"physics_steps_per_action":self.physics_steps_per_action,"disturbance_mode":self.disturbance_mode,"applied_current_ned_mps":config["environment"]["current_mps"],"applied_wind_ned_mps":config["environment"]["wind_mps"],"contract_binding":self.contract_binding}
 ...
 def step(self,action):
  previous_final_distance=math.hypot(self.route[-1][0]-self.last_truth["position_ned_m"][0],self.route[-1][1]-self.last_truth["position_ned_m"][1]);a=np.clip(np.asarray(action,float),-1,1);command={"active_sensors":["imu","gps"],"actuators":{"effectors":{"port":{"command":float(a[0])},"starboard":{"command":float(a[1])}}}};result=None;thrust_samples=[]
  for _ in range(self.physics_steps_per_action):
   result=self.bridge.step([command]);native_info=result["infos"][0];forces=native_info.get("applied_thrust_newtons")
   if forces is None:
    effectors=native_info.get("vehicle_diagnostics",{}).get("effectors",[]);forces=[item.get("thrust") for item in effectors if item.get("id") in ("port","starboard")]
   if len(forces or [])==2:thrust_samples.append(np.asarray(forces,float))
   self.last_observation=result["observations"][0];self.last_truth=self.bridge.ground_truth();self.steps+=1
  ...
  speed=math.hypot(*self.last_truth["velocity_body_mps"][:2]);final_leg_active=self.waypoint==len(self.route)-1;final_distance=math.hypot(self.route[-1][0]-self.last_truth["position_ned_m"][0],self.route[-1][1]-self.last_truth["position_ned_m"][1]);success=final_leg_active and final_distance<=self.final_radius_m;timed_out=self.steps>=self.timeout_steps or bool(result["truncated"][0]);stop_reason=str(result["infos"][0].get("stop_reason","simulator_termination")) if result["terminated"][0] else None;reason=classify_termination(success=success,collision_type=stop_reason if stop_reason in ("grounding","object_collision") else None,allocation_failed=stop_reason=="allocation_failure",unstable=stop_reason=="instability",simulator_terminated=bool(result["terminated"][0]),timed_out=timed_out);terminated=reason not in ("running","timeout");truncated=reason=="timeout";scored=compute_reward(previous_distance,distance,track,a,previous_action,reason,previous_final_distance_m=previous_final_distance,final_distance_m=final_distance,shaping_k=self.shaping_k,shaping_gamma=self.shaping_gamma,shaping_enabled=self.shaping_enabled);reward=scored.reward;completion_fraction=self.completion_tracker.update(scored.progress_reward);policy_thrust=np.mean(thrust_samples,axis=0).tolist() if thrust_samples else None;info={"success":success,"completion_fraction":completion_fraction,"termination_reason":reason,"waypoints_reached":self.waypoint+(1 if success else 0),"current_waypoint_index":self.waypoint,"final_leg_active":final_leg_active,"distance_to_current_waypoint_m":distance,"distance_to_final_waypoint_m":final_distance,"speed_mps":speed,"terminal_definition":{"radius_m":self.final_radius_m,"variant":self.terminal_variant,"timeout_s":self.timeout_steps*self.physics_timestep_s},"reward_components":scored.components(),"mean_cross_track_m":self.cross_track_sum/self.control_steps,"physics_steps":self.steps,"control_steps":self.control_steps,"control_interval_s":self.control_interval_s,"policy_propulsion_thrust_n":policy_thrust,"terminal_state":self.last_truth};return self._obs(),reward,terminated,truncated,info
