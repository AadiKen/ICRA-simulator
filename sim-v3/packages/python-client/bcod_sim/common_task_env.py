...
class CommonWaypointEnv(gym.Env):
 ...
 def _obs(self):
  target=self.route[self.waypoint];imu_payload=self._fresh_sensor("imu");gps_payload=self._fresh_sensor("gps");imu=[0.]*6;gps=[0.]*5
  if imu_payload is not None:imu=[*imu_payload["acceleration_body_mps2"],*imu_payload["angular_rate_body_rad_s"]]
