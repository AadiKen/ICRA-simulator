...
const archive=new NomadsRtofsGridDecoder("20260713T00Z",undefined,async(url)=>{requested=String(url);return new Response(bytes,{status:200});},{root:"s3://noaa-nws-rtofs-pds",mode:"nowcast"});await archive.query({latitude_deg:37.8,longitude_deg:-122.5},"2026-07-13T00:00:00Z");assert.equal(requested,"https://noaa-nws-rtofs-pds.s3.amazonaws.com/rtofs.20260713/rtofs_glo_2ds_n024_prog.nc");
