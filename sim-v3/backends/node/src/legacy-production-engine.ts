...
export interface ProductionAction {waypoints?:Array<{north_m:number;east_m:number}|{lat:number;lon:number}>;active_sensors?:string[];actuators?:{propeller_rps?:number;rudder_rad?:number;surgeForce?:number;yawMoment?:number;desiredWrench?:number[]}}
...
export class LegacyProductionEngine implements SimulationEngine {
  ...
  reset(config:ResolvedExperimentV1):Observation{
    ...
    if(config.vehicle.preset==="searobotics-surveyor-m1.8"){
      validateSurveyorMission(config.mission);const mission=config.mission;
      const bootstrapNewtonsPerCommand=1; // Integration-only. Replace with FIU measured command/thrust curve.
      scenario.boatConfig.vehicleId=SURVEYOR_PUBLIC_SPEC.id;scenario.boatConfig.mass=SURVEYOR_PUBLIC_SPEC.mass_kg;scenario.boatConfig.dimensions=new vec3(SURVEYOR_PUBLIC_SPEC.beam_m,SURVEYOR_PUBLIC_SPEC.draft_m*2,SURVEYOR_PUBLIC_SPEC.length_m);scenario.boatConfig.maxSpeed=3.6;scenario.boatConfig.maxAcceleration=1;scenario.boatConfig.maxTurn=1;scenario.boatConfig.guidanceActuatorMapper=new SurveyorGuidanceMapper({max_thrust_command:mission.max_thrust_command,bootstrap_newtons_per_command:bootstrapNewtonsPerCommand});scenario.boatConfig.productionManifest={vehicle_id:SURVEYOR_PUBLIC_SPEC.id,plant:"planar3",actuation:"twin-fixed-differential",hardware_api:"set_thruster_mode(thrust, thrust_diff)",command_range:[-70,70],calibration_status:SURVEYOR_PUBLIC_SPEC.calibration_status,unresolved:SURVEYOR_PUBLIC_SPEC.unresolved};scenario.boatConfig.vehicleParameters=VehicleParameters.fromGeometry(SURVEYOR_PUBLIC_SPEC.length_m,SURVEYOR_PUBLIC_SPEC.beam_m,SURVEYOR_PUBLIC_SPEC.draft_m,SURVEYOR_PUBLIC_SPEC.mass_kg,{id:SURVEYOR_PUBLIC_SPEC.id,maxAcceleration:1,maxThrust:70*bootstrapNewtonsPerCommand,effectors:[{id:"port",type:"FixedThruster",pos:[0,-.35,0],axis:[1,0,0],dynamics:{tau:.35,min:-70,max:70},conversion:{type:"linear"}},{id:"starboard",type:"FixedThruster",pos:[0,.35,0],axis:[1,0,0],dynamics:{tau:.35,min:-70,max:70},conversion:{type:"linear"}}],controlledDOF:["surge","yaw"],allocator:{mode:"pinv",saturation:"scale"}});
    }
    ...
    this.#sim=new LegacySimulator(scenario);this.#config=config;this.#rng=new SeededRandom(config.experiment.seed);this.#typedOutputs={};this.#typedPowerW.clear();
  ...
  saveState(){this.#ready();return {schema_version:2,simState:structuredClone(this.#sim!.state),actuatorState:this.#sim!.boatModel.savePropulsionState(),rngState:this.#rng!.saveState(),sensorRuntimeState:this.#sensorRuntime!.saveState(),typedOutputs:structuredClone(this.#typedOutputs)};}
  loadState(state:unknown){
    ...
    if(!legacy&&checkpoint.schema_version!==2)throw new Error("Unsupported production-engine checkpoint version.");
    ...
    this.#sim!.state=restored;this.#sim!.boatModel.loadPropulsionState(checkpoint.actuatorState);this.#rng!.loadState(checkpoint.rngState);if(!legacy){this.#sensorRuntime!.loadState(checkpoint.sensorRuntimeState);this.#typedOutputs=structuredClone(checkpoint.typedOutputs??{});}
  }
