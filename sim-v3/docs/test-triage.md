...
| Offline runtime and USV-Bench tests | Pass | Included in 55.08 s | — | Worker runtime, typed positioning, Vehicle C allocation, USV-Bench execution, and the scripted offline four-outcome T8 harness test passed. The revised 84-trial Claude Sonnet 5 run achieved 100% valid completion, invalid safe handling, and ambiguous safe clarification; 83.33% provenance preservation; and a 2.38% silent-error rate (2/84). Both silent-error classifications were D3 trials that reported the correct run and coupled6 validation caveat but omitted the separate metric read-back call. Single model, author-authored tasks, and no ablation remain limitations. |
...
| Focused ICRA TypeScript suite | Pass | Included in 55.08 s | — | Schema, core, vehicle, sensor, environment, metrics, MCP handler, reference, and benchmark tests passed. MCP coverage exercises discovery, validation, deterministic construction, execution, persisted metric read-back, and unknown IDs. |
