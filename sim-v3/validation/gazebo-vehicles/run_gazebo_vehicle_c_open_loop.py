...
def main():
    p=argparse.ArgumentParser();p.add_argument("--schedule",required=True);p.add_argument("--output",required=True);p.add_argument("--world",required=True);a=p.parse_args();samples=json.loads(Path(a.schedule).read_text())["samples"];root=Path(__file__).resolve().parents[2]
    ...
    with tempfile.TemporaryDirectory(prefix="vehicle-c-open-loop-") as td:
      ...
      try:
        ...
        for i,row in enumerate(samples):
          ...
          before=len(odom);target=initial_iteration+(i+1)*5;current=stats[-1].iterations
          if current!=target-5:raise RuntimeError(f"Unexpected pre-step iteration {current}; expected {target-5}")
          node.request("/world/vehicle_c_check/control",WorldControl(pause=True,multi_step=5),WorldControl,Boolean,100)
          ...
          observed=stats[-1];expected_time=(i+1)*.05;elapsed=seconds(observed.sim_time)-initial_sim_time
        ...
        for i,(m,pv,command,iteration,sim_time_s) in enumerate(captured):
          ...
          rows.append({"step":i,"time_s":sim_time_s,"gazebo_iteration":iteration,"gazebo_sim_time_s":sim_time_s,"iteration_delta":5,"sim_time_delta_s":.05,"enu":{"x":m.pose.position.x,"y":m.pose.position.y,"yaw_rad":yaw,"vx":m.twist.linear.x,"vy":m.twist.linear.y,"angular_z":m.twist.angular.z},"command":command,"delivered":{"port_thrust_n":max(MIN_THRUST,min(MAX_THRUST,command["port_thrust_n"])),"port_azimuth_rad":angles["port"],"starboard_thrust_n":max(MIN_THRUST,min(MAX_THRUST,command["starboard_thrust_n"])),"starboard_azimuth_rad":angles["starboard"]}})
        out=Path(a.output);out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps({"schema_version":2,"synchronization":{"source":"/world/vehicle_c_check/stats","initial_iteration":initial_iteration,"initial_sim_time_s":initial_sim_time,"target_policy":"absolute initial_iteration + 5*(sample_index+1)","blind_retries":False,"asserted_iteration_delta":5,"asserted_sim_time_delta_s":.05},"rows":rows,"delivery_basis":{"thrust":"direct cmd_thrust force after corrected SDF clipping","azimuth":"measured base-to-pod relative pose from temporary SceneBroadcaster instrumentation"}},indent=2)+"\n")
      finally:
