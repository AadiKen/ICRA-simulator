...
assert.equal(artifact.failure_injection.length,31,"Six sources x five transport/shape failures plus NDBC sentinel-only poisoning must be covered.");
...
assert.ok(artifact.bathymetry_fixture_differences.every((row:any)=>Number.isFinite(row.rms_fixture_difference_m)&&Number.isFinite(row.mean_fixture_difference_m)&&row.evidence_class.includes("not measured")));
assert.match(artifact.methodology.bathymetry_cascade,/do not have live acquisition adapters/);
