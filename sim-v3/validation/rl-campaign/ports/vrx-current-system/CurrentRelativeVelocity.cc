...
class CurrentRelativeVelocity final : public gz::sim::System,
 ...
 public:
  void Configure(const gz::sim::Entity &entity,
    ...
    this->mass=sdf->Get<double>("mass",52.3).first;
  ...
  void PreUpdate(const gz::sim::UpdateInfo &info, gz::sim::EntityComponentManager &ecm) override {
    ...
    const double yawRate=bodyAngularVelocity.Z();
    const double addedMassForceX=-this->yvDot*relativeV*yawRate-
      (-this->yvDot*bodyVelocity.Y()*yawRate);
    const double addedMassForceY=this->xuDot*relativeU*yawRate-
      (this->xuDot*bodyVelocity.X()*yawRate);
    const gz::math::Vector3d correctionBody(
      damp(relativeU,this->xu,this->xuu)-damp(bodyVelocity.X(),this->xu,this->xuu)+addedMassForceX,
      damp(relativeV,this->yv,this->yvv)-damp(bodyVelocity.Y(),this->yv,this->yvv)+addedMassForceY,0);
    ...
    // DART multiplies C_RB(nu) by absolute velocity. Node builds C_RB from
    // absolute velocity but multiplies it by current-relative velocity.
    const double rigidBodyMomentZ=this->mass*(
      bodyVelocity.Y()*bodyCurrent.X()-bodyVelocity.X()*bodyCurrent.Y());
    const double totalMomentZ=currentMomentZ+rigidBodyMomentZ;
    ...
    hull.AddWorldWrench(ecm,correctionWorld+pose->Rot().RotateVector(driveBody),
      pose->Rot().RotateVector(gz::math::Vector3d(0,0,totalMomentZ)));
    if(this->diagnostic.is_open()) {
      ...
      this->diagnostic << time << ',' << bodyVelocity.X() << ',' << bodyVelocity.Y()
        ...
        << ',' << totalMomentZ << ',' << addedMassEigenvalueMin << ','
 ...
 private:
  ...
  double xuDot{-2.615},yvDot{-39.225},mass{52.3};
