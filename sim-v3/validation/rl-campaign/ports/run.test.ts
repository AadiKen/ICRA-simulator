...
import {losAction,losDesiredHeading,vehicleAWrenchToActuators} from "./portable-controllers.ts";
const action=losAction("LOS-PID-v2",{north_m:0,east_m:0,heading_rad:0,surge_mps:0,yaw_rate_rad_s:0},[0,0],[20,0]);
const capture=losDesiredHeading({north_m:21,east_m:2,heading_rad:0,surge_mps:0,yaw_rate_rad_s:0},[0,0],[20,0],8);assert.equal(capture.mode,"point-capture");assert.ok(Math.abs(capture.desired_rad-Math.atan2(-2,-1))<1e-12);
