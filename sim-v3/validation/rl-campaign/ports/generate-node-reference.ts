...
export function generateNodeReference(seed:number,out:string,steps=FULL_STEPS,scales={wind:1,current:1,action:1}){
 ...
 for(let step=0;step<steps;step++){const a=commandAt(Math.floor(step/2)*2),result:any=sim.step({active_sensors:["imu","gps"],actuators:{effectors:{port:{command:scales.action*a[0]},starboard:{command:scales.action*a[1]}}}}),g:any=sim.getGroundTruth(),imu=result.observation.sensors.imu,gps=result.observation.sensors.gps;odom.push({time_s:step*PHYSICS_DT,N_m:g.position_ned_m[0],E_m:g.position_ned_m[1],yaw_rad:g.attitude_rad[2],u_mps:g.velocity_body_mps[0],v_mps:g.velocity_body_mps[1],r_rad_s:g.angular_rate_body_rad_s[2],imu:imu?{timestamp_s:imu.timestampS,valid:imu.valid,linear_accel_body:imu.payload?.acceleration_body_mps2,angular_rate_body:imu.payload?.angular_rate_body_rad_s,yaw_ned_rad:imu.payload?.orientation_rad?.[2]}:undefined,gps:gps?{timestamp_s:gps.timestampS,valid:gps.valid,position_ned_m:gps.payload?.position_ned_m?.slice(0,2)}:undefined});}
