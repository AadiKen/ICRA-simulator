...
export const TRACE_SCHEMA_V2={
  ...
  task_contract:{task_id:"common-waypoint-transit-v1",content_sha256:"2eff3e87da1c789f048711faf42972d7b66b130a939878a0a80d1b778924bb36"},
...
export function assertTraceV2(trace:TraceV2):void {
  ...
  for(const [index,sample] of trace.samples.entries()){
    ...
    if(sample.state.length!==6||sample.observation.length!==15||sample.applied_action.length!==4||command.command.length!==4) throw new Error(`Invalid trace field width at step ${index}.`);
    if(sample.observation[14] < 0 ||sample.observation[14] > 1) throw new Error(`Invalid normalized time remaining at step ${index}.`);
