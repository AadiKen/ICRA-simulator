...
for line in sys.stdin:
 try:
  ...
  if op=="publish":
   ...
   message=WorldControl();message.pause=True;message.multi_step=int(request.get("multi_step",0));executed,result=node.request(request["service"],message,WorldControl,Boolean,10000);response={"ok":bool(executed and result.data)}
