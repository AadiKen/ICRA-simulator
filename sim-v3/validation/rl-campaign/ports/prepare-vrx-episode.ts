...
export function prepareVrxEpisode(seed:number,out:string,steps=FULL_STEPS,environmentScale=1,actionScale=1,windScale=environmentScale,currentScale=environmentScale,collisionMode="none",surfaceMode="both",leanTransport=false){
...
if(process.argv[1]===import.meta.filename){const[seed,out,environmentScale="1",actionScale="1",windScale=environmentScale,currentScale=environmentScale,collisionMode="none",surfaceMode="both",leanTransport="0"]=process.argv.slice(2);if(!seed||!out)throw new Error("usage: prepare-vrx-episode.ts seed output-directory [environment-scale] [action-scale] [wind-scale] [current-scale] [collision-mode] [surface-mode] [lean-transport]");console.log(JSON.stringify(prepareVrxEpisode(Number(seed),out,FULL_STEPS,Number(environmentScale),Number(actionScale),Number(windScale),Number(currentScale),collisionMode,surfaceMode,leanTransport==="1"),null,2));}
