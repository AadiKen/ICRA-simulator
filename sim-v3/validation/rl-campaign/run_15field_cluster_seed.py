...
def main():
 ...
 try:
  for checkpoint in range(250_000,1_500_001,250_000):
   ...
   for row in result['rows']:row.update({'host_class':'cluster','observation_field_count':15,'contract_sha256':CONTRACT})
   curve.append({'checkpoint_steps':checkpoint,'actual_model_timesteps':model.num_timesteps,'evaluation':result});atomic_json(out/'report.json',{'schema_version':1,'artifact_kind':'15-field-cluster-recurrent-ppo-seed','status':'running','host_class':'cluster','contract_sha256':CONTRACT,'observation_schema':FIELDS,'training':{'seed':seed,'algorithm':'RecurrentPPO','policy':'MlpLstmPolicy','steps':1_500_000,'parallel_environments':n_env,'n_steps':512,'batch_size':512,'n_epochs':10,'learning_rate':3e-4,'gamma':.99,'gae_lambda':.95,'clip_range':.2,'ent_coef':.005,'curriculum':'final-leg-isolation'},'curve':curve})
