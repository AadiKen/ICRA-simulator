...
PROTOCOL_PATH=ROOT/"artifacts/rl-campaign/surveyor/p3-v6-feedforward-training-protocol.json"
OUT=ROOT/"artifacts/rl-campaign/surveyor/p3-v6-feedforward-local"
CONTRACT=json.loads(CONTRACT_PATH.read_text())
PROTOCOL=json.loads(PROTOCOL_PATH.read_text())
assert CONTRACT["content_sha256"]==PROTOCOL["task_contract_content_sha256"]
if not PROTOCOL["execution_authorized"]:raise RuntimeError("Long run disabled: set execution_authorized only when explicitly starting P3-v6 feedforward.")
...
report={"schema_version":1,"artifact_kind":"surveyor-p3-v6-feedforward-curriculum-gate-3","task_contract_content_sha256":CONTRACT["content_sha256"],"training":{"phase_1_steps":phase1[-1]["checkpoint_steps"],"phase_2_steps":3_000_000,"seed":7319,"wall_clock_s":time.time()-started},"evaluation":{**gate,"normalized_base_return":normalized},"acceptance":{"median_base_return_min":return_rule["value"],"normalized_base_return_min":.5,"success_rate_min":success_rule["value"],"both_must_pass":True},"gate_passed":passed,"no_fifth_revision_without_diagnosis":True};atomic(OUT/"report.json",report);atomic(OUT/"training-state.json",{"status":"completed","gate_passed":passed,"report":"artifacts/rl-campaign/surveyor/p3-v6-feedforward-local/report.json"});raise SystemExit(0 if passed else 2)
