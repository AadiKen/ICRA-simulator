#!/usr/bin/env python3
"""Mechanical PPO smoke test for the validated HoloOcean Vehicle A path.
...
PROTOCOL_PATH = SOURCE_ROOT / "artifacts/rl-campaign/surveyor/portable-feedforward-training-protocol.json"
...
def main() -> None:
    ...
    if (
        protocol.get("task_contract_content_sha256") != FROZEN_CONTRACT_SHA256
        or not protocol.get("execution_authorized", False)
    ):
        raise RuntimeError("feedforward smoke execution is not authorized by its protocol")
    ...
    try:
        ...
        callback = CheckpointCallback(
            ...
            name_prefix="holoocean-feedforward-smoke",
        )
        ...
        final_path = args.output / "feedforward-ppo-smoke-final"
    ...
    loaded = PPO.load(load_target, device="cuda")
    ...
    result = {
        "schema_version": 1,
        "artifact_kind": "holoocean-vehicle-a-feedforward-ppo-smoke",
        "status": "passed",
